...
func (c *UploadController) Post() {
	...
	if c.walletClient == nil {
		isEnableCrypto, _ := beego.AppConfig.Bool("crypto::enable")
		config := restapi.Config{
			...
			CryptoCfg: &restapi.CryptoConfig{
				Enable:         isEnableCrypto,
				CertsStorePath: beego.AppConfig.String("crypto::certspath"),
			},
			EnterpriseSignParam: &pki.SignatureParam{
				Creator:    did.Identifier(beego.AppConfig.String("signature::creator")),
				Nonce:      beego.AppConfig.String("signature::nonce"),
				PrivateKey: beego.AppConfig.String("signature::privatekey"),
			},
		}
